...
def expected_values() -> tuple:
    ...
    return int_cubic_all_real_roots, int_cubic_one_real_two_complex_roots, int_quartic_all_real_roots, \
           int_quartic_two_real_two_complex_roots
...
if __name__ == "__main__":
    start = time.time()
    ffr = 1.0
    cubic_all_real_roots, cubic_one_real_two_complex_roots, quartic_all_real_roots, quartic_two_real_two_complex \
        = expected_values()
    ...
    if comparison_result_floats(integrals.elliptical_integral_cubic_one_real_and_two_complex_roots
                                ...
                                cubic_one_real_two_complex_roots, eps=eps):
    ...
    eps = 1e-6
    if comparison_result_floats(integrals.elliptical_integral_quartic_all_roots_real
                                    (p_test, a_test, b_test, ffr, y_test, x_test) / 2.0,
                                quartic_all_real_roots, eps=eps):
    ...
    if comparison_result_floats(integrals.elliptical_integral_quartic_two_real_and_two_complex_roots
                                    (p_test, a_test, b_test, fgh_test, ffr, y_test, x_test) / 4.0,
                                quartic_two_real_two_complex, eps=eps):
